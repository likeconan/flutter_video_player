...
extension PlayerView {
    ...
    private func setUpAsset(with u: URL, completion: ((_ asset: AVAsset) -> Void)?) {
        ...
        if(!url.path.hasSuffix(".m3u8") && FileManager.default.fileExists(atPath: url.path)) {
            ...
            self.loaderDelegate = CacheResourceLoaderDelegate(withURL: u, urlRequested: {url in
                self.delegate?.onUrlRequested(url: url)
            })
    ...
    func seekTo(time:CMTime) {
        ...
        var canSeek = true;
        if let seeked = seekedTime {
            let seconds = CMTimeGetSeconds(seeked)
            canSeek = self.currentTime - seconds >= 1 && seconds != CMTimeGetSeconds(time);
        }
        if(!canSeek){
            return;
        }
